...
const TRANSACTION_TYPES = ["ingreso", "egreso", "transferencia"] as const;
...
interface ModelOutput extends DetectedTransaction {
...
function weekdayInBogota(): string {
 ...
 */
function recentDaysReference(days = 10): string {
...
function sanitize(
  ...
  if (
    ...
    const parsed = new Date(`${raw.date}T00:00:00`);
    if (!Number.isNaN(parsed.getTime())) date = raw.date;
  }
...
function readCatalog<T extends { id: unknown }>(value: unknown): T[] {
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    return Response.json({
      text,
      detected: sanitize(parsed, text, categories, accounts),
    });
  } catch (error) {
